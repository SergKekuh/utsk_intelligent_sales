...
# v3.4 — исправленный порядок: проверка аргументов → копирование → загрузка
# ================================================================
set -e
...
echo "=== ЗАГРУЗКА $TARGET_YEAR v3.4 ==="

# Проверка файлов
if [ ! -f "$SALES_CSV" ]; then
    echo "❌ Файл продаж не найден: $SALES_CSV"
    exit 1
fi
if [ ! -f "$STOCK_CSV" ]; then
    echo "❌ Файл остатков не найден: $STOCK_CSV"
    exit 1
fi

# --- 3. КОПИРОВАНИЕ И КОНВЕРТАЦИЯ ---
echo "[1/4] Копирование и конвертация CSV за $TARGET_YEAR год..."
mkdir -p "$TMP_DIR"

cp "$SALES_CSV" /tmp/unified_sales_$TARGET_YEAR.csv
cp "$STOCK_CSV" /tmp/stock_balance_$TARGET_YEAR.csv

sed 's/,/./g' /tmp/unified_sales_$TARGET_YEAR.csv | iconv -f WINDOWS-1251 -t UTF-8 > "$TMP_DIR/sales.csv" 2>/dev/null || sed 's/,/./g' /tmp/unified_sales_$TARGET_YEAR.csv > "$TMP_DIR/sales.csv"
sed 's/,/./g' /tmp/stock_balance_$TARGET_YEAR.csv | iconv -f WINDOWS-1251 -t UTF-8 > "$TMP_DIR/stock.csv" 2>/dev/null || sed 's/,/./g' /tmp/stock_balance_$TARGET_YEAR.csv > "$TMP_DIR/stock.csv"
...
# --- 5. АНАЛИТИКА ---
echo "[3/4] Пересчёт аналитики за $TARGET_YEAR..."
sudo -u "$DB_USER" psql -d "$DB_NAME" <<SQL
...
# --- 6. ПРОВЕРКА ABC ---
echo "[4/4] Проверка ABC за $TARGET_YEAR..."
